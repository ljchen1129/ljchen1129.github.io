...
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170408_1.png?imageView/0/h/300)
...
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170408_2.png?imageView/0/h/500)
...
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170410_4.png?imageView/0/h/350)
...
```
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170410_5.png?imageView/0/h/200)
...
1. 如果将一个对象添加到一个数组中，那么数组会对对象进行一次 retain
	...
	![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170411_10.png?imageView/0/h/250)
	
2. 当数组对象释放后，会给数组中所有的对象发送一条 release 消息

	![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170411_11.png?imageView/0/h/250)

3. 当数组移除一个对象之后，会给对象发送一条 release 消息
	
	![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170411_12.png?imageView/0/h/250)
...
- 几种情况分析
	1. 可变对象调用 copy
		...
		![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_2.png?imageView/0/h/230)
	
	>结论： `可变对象`调用 copy 方法，`会生成`新的对象，并且新对象是`不可变`的。
	
	2. 可变对象调用 mutableCopy
	
		![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_3.png?imageView/0/h/240)
	
	>结论： `可变对象`调用 mutableCopy，`会生成`新的对象，并且新对象是`可变的`。
	
	3. 不可变对象调用 mutableCopy
	
		![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_4.png?imageView/0/h/240)
	
	>结论： `不可变对象`调用 mutableCopy，`会生成`新的对象，并且新对象是`可变的`。
	
	4. 不可变对象调用 copy
	
		![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_5.png?imageView/0/h/230)
...
- 如果是浅拷贝（源对象和副本对象都不可变），由于不会生成新的对象，所以系统会对源对象进行一次 retain。所以对源对象一次 copy 操作，就要相应的做一次 release 操作。
	...
	![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_6.png?imageView/0/h/230)

- 如果是深拷贝。真是因为生成了新的对象，所以系统不会对源对象进行 retain，但是因为生成了新对象，所以需要对新对象进行一次 release。

	![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_8.png?imageView/0/h/260)
...
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_9.png?imageView/0/h/260)


#### #修饰 block
- 如果 block 存储在`栈`中，block 中访问了外界的对象，`不会`对对象进行一次 retain 操作
	
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_16.png?imageView/0/h/220)
		
- 如果 block 存储在`堆`中，block 中访问了外界的对象，`会`对对象进行一次 retain 操作
	
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_15.png?imageView/0/h/220)
...
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_17.png?imageView/0/h/250)
...
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_18.png?imageView/0/h/240)
...
![](https://blogimages-1254431338.cos.ap-shenzhen-fsi.myqcloud.com/Snip20170412_19.png?imageView/0/h/240)
